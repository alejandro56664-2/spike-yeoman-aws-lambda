...
module.exports = class extends FnGenerator {
  ...
  prompting() {
    ...
    return this.prompt(prompts).then(props => {
      ...
      this.props.functionNameCamelCase = this._toCamelCase(this.props.functionName)
      ...
      let functionsList = this.config.get("functionsList") || []
  ...
  initializing() {
    ...
    this.composeWith(require.resolve('../config-manager-ssm'), {invokedFromFnController: true });
    //permite gestionar los diferentes servicios de backend disponibles para la función
    this.composeWith(require.resolve('../backend'), { invokedFromFnController: true });
    this.composeWith(require.resolve('../svc'), { invokedFromFnController: true});
  }
  ...
  writing() {
    ...
    this._appendHelper(this.props, 'settings-snippet.gradle', 'settings.gradle')
  ...
  _copySrc(props) {
    this.log(`Copiando archivos fuente para la función, con el paquete base: ${props.pkgBase}`)
    ...
    // este método incluye los objetos hijos del request, los cuales tienen por convención un sufijo con el tipo de 'trigger'
    const allRequestObjPaths = this._getAllRequestObj('src/main/java/pkgBase', this.props.trigger)
    ...
    this._copySrcWithOtherNameHelper(props, 'src/main/java/pkgBase', filesMain, this._removeNameSuffix)
  ...
  _getAllRequestObj(prefixPath, trigger){
    ...
    return [
      `model/request/Request-${trigger}.java`
    ] 
  }
